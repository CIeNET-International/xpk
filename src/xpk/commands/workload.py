...
def workload_create(args) -> None:
  ...
  if getattr(args, 'virtual_slices', None):
      xpk_print(f"Injecting virtual_slices into command: {args.virtual_slices}")
      args.command = f"{args.command} --virtual_slices={args.virtual_slices}"
